...
<? if ($arResult["isFormNote"] != "Y") {
    ...
    foreach ($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion) {
        ...
        <?
            ...
            ?>
            <?
            if ($arQuestion['STRUCTURE'][0]['ID'] != 14 && $arQuestion['STRUCTURE'][0]['ID'] != 29) {
                ?>
                <?
            ...
            <?
            } elseif ($arQuestion['STRUCTURE'][0]['ID'] == 14) {
                ?>
                <input type="hidden" class="inputtext" name="form_text_14" value="<?= $doctorEmail ?>" size="0">
            <?
            } elseif ($arQuestion['STRUCTURE'][0]['ID'] == 29) {
                ?>
                <input type="hidden" class="inputtext" name="form_text_29" value="<?= $doctorPhone ?>" size="0">
            <?
            } ?>
            <?
        }

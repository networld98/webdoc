            ...
            <div class="clinic-card-item card-item">
                <div class="clinic-card-img">
                    <div class="clinic-card-im-flex-right">
                        ...
                        <div class="clinic-card-img__ratings">
                            <img src="<?= SITE_TEMPLATE_PATH ?>/assets/images/<? if ($arRaing['RATING'] >= '1') {
                                ?>filled-star.svg<? } else {
                                ?>unfilled-star.svg<? } ?>" alt="star">
                            <img src="<?= SITE_TEMPLATE_PATH ?>/assets/images/<? if ($arRaing['RATING'] >= '2') {
                                ?>filled-star.svg<? } else {
                                ?>unfilled-star.svg<? } ?>" alt="star">
                            <img src="<?= SITE_TEMPLATE_PATH ?>/assets/images/<? if ($arRaing['RATING'] >= '3') {
                                ?>filled-star.svg<? } else {
                                ?>unfilled-star.svg<? } ?>" alt="star">
                            <img src="<?= SITE_TEMPLATE_PATH ?>/assets/images/<? if ($arRaing['RATING'] >= '4') {
                                ?>filled-star.svg<? } else {
                                ?>unfilled-star.svg<? } ?>" alt="star">
                            <img src="<?= SITE_TEMPLATE_PATH ?>/assets/images/<? if ($arRaing['RATING'] >= '5') {
                                ?>filled-star.svg<? } else {
                                ?>unfilled-star.svg<? } ?>" alt="star">
                        </div>
                        <a class="clinic-card-img__link"
                           href="<?= $arItem['DETAIL_PAGE_URL'] ?>#otzivy-yakor"><? getTermination($arRaing['COUNT']) ?></a>
                    </div>
                        ...
                        <div class="adapt-clone-2">
                            <div>
                                <div class="clinic-card-img__img">
                                    <a href="<?= $arItem['DETAIL_PAGE_URL'] ?>">
                                        ...
                                        <? } else {
                                            ?>
                                            <img src="<?= SITE_TEMPLATE_PATH ?>/icon/hospital_building.svg"
                                                 alt="нет лого">
                                        <? } ?>
                                    </a>
                                </div>
                            ...
                            <div>
                                ...
                                <? if ($arItem["DISPLAY_PROPERTIES"]["COST_PRICE"]["DISPLAY_VALUE"]):?>
                                    <p class="clinic-card-desc__price">Первичная стоимость приёма -
                                        <span><?= $arItem["DISPLAY_PROPERTIES"]["COST_PRICE"]["DISPLAY_VALUE"] ?></span>
                                    </p>
                                <? endif; ?>
                ...
                <div class="clinic-card-desc">
                    <div class="clinic-card-desc-hide">
                        ...
                        <? if ($arItem["DISPLAY_PROPERTIES"]["COST_PRICE"]["DISPLAY_VALUE"]):?>
                            <p class="clinic-card-desc__price">Первичная стоимость приёма -
                                <span><?= $arItem["DISPLAY_PROPERTIES"]["COST_PRICE"]["DISPLAY_VALUE"] ?></span></p>
                        <? endif; ?>
                        <?
                        $res = CIBlockElement::GetByID($arItem["PROPERTIES"]["MAIN_SPECIALIZATION"]["VALUE"]);
                        if ($ar_res = $res->GetNext()) {
                    ...
                    <? if ($arItem["PROPERTIES"]["MAP"]["VALUE"]):?>
                        <div class="map-wrapper">
                            ...
                            <div class="popup-box">
                                <div class="close"></div>
                                <div class="map-popup-marker" id="map_<?= $arItem['ID'] ?>"
                                     style="width: 100%;height:500px;"></div>
                                <div class="map-popup" id="map_track_<?= $arItem["ID"] ?>"
                                     style="width: 100%;height:500px;"></div>
                    ...
                    <div class="grey-block">
                        <div class="clinic-card-info__block">
                            <? if ($arItem["DISPLAY_PROPERTIES"]["CITY"]["DISPLAY_VALUE"]):?>
                                ...
                                <span><? if ($arItem["DISPLAY_PROPERTIES"]["REGION"]["DISPLAY_VALUE"]) {
                                        ?><?= $arItem["DISPLAY_PROPERTIES"]["REGION"]["DISPLAY_VALUE"] ?>, <? } ?><? if ($arItem["DISPLAY_PROPERTIES"]["CITY"]["DISPLAY_VALUE"]) {
                                        ?>г. <?= $arItem["DISPLAY_PROPERTIES"]["CITY"]["DISPLAY_VALUE"] ?>, <? } ?><? if ($arItem["DISPLAY_PROPERTIES"]["AREA"]["DISPLAY_VALUE"]) {
                                        ?><?= $arItem["DISPLAY_PROPERTIES"]["AREA"]["DISPLAY_VALUE"] ?>, <? } ?><? if ($arItem["DISPLAY_PROPERTIES"]["ADDRESS"]["DISPLAY_VALUE"]) {
                            ...
                            <? if ($arItem["DISPLAY_PROPERTIES"]["WORK_TIME"]["DISPLAY_VALUE"] != NULL):?>
                                ...
                                <? else:?>
                                    ...
                                    foreach ($work_time as $day) {
                                        ...
                                        <? } elseif (count($time) == 5) {
                                            ?>
                                            <span><span class="work-day"><?= $time[0] ?></span> -  c <?= $time[1] ?>
                                                :<?= $time[2] ?> до <?= $time[3] ?>:<?= $time[4] ?></span>
                                        <? } ?>
                                    <? } ?>
                                <? endif; ?>
                            <? endif; ?>
